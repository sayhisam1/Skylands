--- Bind class to Roblox Instance (adopted from https://github.com/Quenty/NevermoreEngine/blob/version2/Modules/Shared/Binder/Binder.lua)
-- @classmod Binder
...
function Binder.new(tagName, constructor)
	local self = setmetatable(BaseObject.new(tagName), Binder)
...
function Binder:GetClassAddedSignal()
	...
	self._classAddedSignal = Event.new() -- :fire(class, inst)
...
function Binder:GetClassRemovingSignal()
	...
	self._classRemovingSignal = Event.new() -- :fire(class, inst)
