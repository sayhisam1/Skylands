...
--- Returns the last recorded ping
-- @treturn number latency
...
function SlaveClock.new(remoteEvent, remoteFunction)
	...
	self._remoteEvent.OnClientEvent:Connect(
		function(timeOne)
...
function SlaveClock:_handleSyncEvent(timeOne)
	...
	local ping = (masterSlaveDifference + slaveMasterDifference) / 2

	self._offset = offset -- Estimated difference between server/client
	self._ping = ping -- Estimated time for network events to send. (MSDelay/SMDelay)
end
...
local function buildClock()
	...
	if IsServer then
		...
		clockDir.Parent = game:GetService("ReplicatedStorage")
		remoteEvent = Instance.new("RemoteEvent")
		remoteEvent.Name = "TimeSyncEvent"
		remoteEvent.Parent = clockDir
		remoteFunction = Instance.new("RemoteFunction")
		remoteFunction.Name = "DelayedRequestEvent"
		remoteFunction.Parent = clockDir
	end
	clockDir = game:GetService("ReplicatedStorage"):WaitForChild("_clockRemote")
	remoteEvent = clockDir:WaitForChild("TimeSyncEvent")
	remoteFunction = clockDir:WaitForChild("DelayedRequestEvent")
