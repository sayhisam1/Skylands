		...
		if childPlanNode.type == TestEnum.NodeType.It then
			if session:shouldSkip() then
				...
				if tryStack:size() > 0 and tryStack:getBack().isOk == false then
					...
					table.insert(
						childResultNode.errors,
						string.format(
							"%q failed without trying, because test case %q failed",
							childPlanNode.phrase,
							tryStack:getBack().failedNode.phrase
						)
					)
				else
					...
					local nodeSuccess, nodeResult =
						call(
						childPlanNode.callback,
						function(message)
							return message .. "\n" .. debug.traceback()
						end
					)
