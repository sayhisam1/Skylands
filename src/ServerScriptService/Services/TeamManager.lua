...
function Service:Load()
    ...
    -- local SPAWN_LOCATIONS = workspace:WaitForChild("Map"):WaitForChild("DEFAULT_SPAWNS"):GetChildren()
    -- local spawns = DEFAULT_TEAM.Spawns
    -- for _, spawn in pairs(SPAWN_LOCATIONS) do
        -- spawns:AddSpawn(spawn)
    -- end
    DEFAULT_BOT_TEAM = Team:New("DEFAULT_BOT", 2)
    self:AddTeam(DEFAULT_TEAM)
    self:AddTeam(DEFAULT_BOT_TEAM)
    DEFAULT_TEAM:AddHostileTeam(DEFAULT_BOT_TEAM)
end
